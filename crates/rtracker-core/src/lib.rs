...
pub use presets::{Category as InstrumentCategory, Instrument};
...
pub enum ValidationError {
    ...
    #[error("voice '{voice}' sine_partials must contain at least one partial")]
    EmptySinePartials { voice: String },
    #[error("voice '{voice}' sine_partials has {partials} partials but {amplitudes} amplitudes")]
    BadSinePartialsLength { voice: String, partials: usize, amplitudes: usize },
    #[error("voice '{voice}' sine_partials partial {index} has invalid value {partial} (must be finite and > 0)")]
    BadSinePartial { voice: String, index: usize, partial: f32 },
    #[error("voice '{voice}' sine_partials amplitude {index} is not finite")]
    BadSineAmplitude { voice: String, index: usize },
}
            ...
            match v {
                ...
                VoiceDef::SinePartials { partials, amplitudes, .. } => {
                    if partials.is_empty() {
                        return Err(ValidationError::EmptySinePartials { voice: name.clone() });
                    }
                    if partials.len() != amplitudes.len() {
                        return Err(ValidationError::BadSinePartialsLength {
                            voice: name.clone(),
                            partials: partials.len(),
                            amplitudes: amplitudes.len(),
                        });
                    }
                    for (index, partial) in partials.iter().copied().enumerate() {
                        if !partial.is_finite() || partial <= 0.0 {
                            return Err(ValidationError::BadSinePartial {
                                voice: name.clone(),
                                index,
                                partial,
                            });
                    ...
                    for (index, amplitude) in amplitudes.iter().enumerate() {
                        if !amplitude.is_finite() {
                            return Err(ValidationError::BadSineAmplitude {
                                voice: name.clone(),
                                index,
                            });
                        }
...
mod tests {
    ...
    #[test]
    fn validation_rejects_mismatched_sine_partials() {
        let mut voices = HashMap::new();
        voices.insert(
            "partials".into(),
        ...
        assert!(matches!(
            p.validate(),
            Err(ValidationError::BadSinePartialsLength { .. })
        ));
    }

    #[test]
    fn validation_rejects_empty_sine_partials() {
        let mut voices = HashMap::new();
        voices.insert(
            "partials".into(),
            VoiceDef::SinePartials {
        ...
        assert!(matches!(
            p.validate(),
            Err(ValidationError::EmptySinePartials { .. })
        ));
    }
}
